...
- 📂 **自动展开折叠章节组**：切到下一章节前，如果目标章节组折叠，会先点开它，避免跳过去后看不到自己在哪
- ⏭ **自动连播**：开启后，视频播放完成后自动调用"下一个"逻辑切到下一节
- 💾 **状态持久化**：两个开关的状态（"上下切换按钮"、"自动连播"）通过 `chrome.storage.sync` 跨设备同步
- 🔌 **可插拔**：关闭"上下切换按钮"开关后按钮从控制栏移除，关闭"自动连播"开关后停止监听 ended 事件
...
├── popup.html             # 弹窗 UI（含两个开关：上下切换按钮、自动连播）
...
- **自动展开折叠章节组**：如果目标 li 所在的 `div.collapse` 章节组未展开，先点击 `.collapse-title` 展开，再点击 li（避免跳到新章节后右边目录还是折叠的看不到位置）
- 切换期间锁定按钮，等待 `playing.gif` 移到目标 li 后再解锁（最多 5 秒超时兜底）
- **`handleNext` 复用**：按钮点击和视频 ended 事件共用同一套切换逻辑
- **自动连播**：开启后通过 `MutationObserver` 监听 `<video>` 标签出现，绑定 `ended` 事件，触发时调用 `handleNext()`
